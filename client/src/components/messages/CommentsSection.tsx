    ...
    <div className="mt-3 border-t border-[#e3cac0]/60 pt-3">
            ...
            return (
              <div key={comment.id} className="flex gap-2 items-start group">
                <Avatar className="h-5 w-5 flex-shrink-0 mt-0.5">
                  ...
                  <AvatarFallback className="bg-[#263d57] text-white text-[8px]">
                    {getInitials(comment.authorFirstName, comment.authorLastName, comment.authorDisplayName)}
                  </AvatarFallback>
                </Avatar>
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline gap-1.5 flex-wrap">
                    <span className="text-[11px] font-semibold text-[#263d57]">
                      {getAuthorName(comment)}
                    </span>
                    <span className="text-[10px] text-[#263d57]/35">
                      {formatDistanceToNow(new Date(comment.createdAt), { addSuffix: true })}
                    </span>
                  </div>
                  <p className="text-[11px] text-[#263d57]/80 leading-snug break-words">
                    {comment.content}
                  </p>
                </div>
                {isOwn && (
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0 p-0.5 rounded hover:bg-red-50 text-red-400 hover:text-red-600"
      ...
      <div className="flex gap-2 items-end">
        <Textarea
          ...
          rows={1}
          className="min-h-0 h-8 py-1.5 px-2.5 text-xs resize-none border-[#e3cac0] focus:border-[#b95827] focus-visible:ring-[#b95827]/20 rounded-lg flex-1"
          maxLength={1000}
        />
        <Button
          size="sm"
          ...
          className="h-8 w-8 p-0 bg-[#b95827] hover:bg-[#a04d22] flex-shrink-0"
